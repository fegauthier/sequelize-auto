...
AutoSequelize.prototype.ensurePrimaryKey = function(){
  //Ensure the filed id is a primaryKey cause it's false for views by default
  ...
  Object.keys(self.tables).forEach(function(tableName){
    ...
    //TODO test if there is already a primaryKey in the table

    Object.keys(table).forEach(function(columnName){
      var column = table[columnName];

      if(columnName == "id"){
        _tables[tableName][columnName] = _.assign({}, column, {
          primaryKey: true
        })
      } else {
        _tables[tableName][columnName] = column;
      }
    })
  })
